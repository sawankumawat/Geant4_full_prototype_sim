...
G4double Geometry::ConstructFoCalE(G4LogicalVolume *envelope)
{
    ...
    // G4Box* SCI_Absorber 	= new G4Box("PSCI_Absorber",	PAD_MiniFoCal_X/1.5, PAD_MiniFoCal_Y/1.5, PAD_Absorber_Z/1.333); // change here for the width of the plastic scintillator
    ...
    G4AssemblyVolume *assemblyPad1 = ConstructPadLayer1();
    ...
    for (G4int ilayer = 0; ilayer < NumberOfLayers; ilayer++)
    {
        ...
        else if (LayerLayout[ilayer] == 2)
        {
            // G4double position_Z_PAD_Absorber = Start_Z + PAD_Absorber_Z/2. + TotalLayerThickness;
            // G4double position_Z_PAD_Absorber = 225 * mm + PAD_Absorber_Z / 2. + TotalLayerThickness;
            // G4double position_Z_PAD_Absorber = 225 * mm + PAD_Absorber_Z / 2. + TotalLayerThickness;

            // G4double position_Z_PAD	     = Start_Z + PAD_Absorber_Z + TotalLayerThickness;
            // G4double position_Z_PAD	     =  Start_Z + PAD_Absorber_Z/2. + PAD_Absorber_Z_Last;
            G4double position_Z_PAD = Start_Z + PAD_Absorber_Z_Last + TotalLayerThickness;
            ...
            //  G4double position_Z_SCI21 = +217 +30 * mm; // vertical PS
            // G4double position_Z_SCI22 = +222 +30 * mm; // horizontal PS
            ...
            new G4PVPlacement(0, G4ThreeVector(Start_X, Start_Y + 15 * mm, position_Z_SCI21), logVol_SCI_Absorber1, "PhysVol_S21", envelope, false, 0); // vertical PS
            new G4PVPlacement(0, G4ThreeVector(Start_X, Start_Y + 15 * mm, position_Z_SCI22), logVol_SCI_Absorber2, "PhysVol_S22", envelope, false, 0); // horizontal PS

            G4cout << ilayer << " PAD at " << position_Z_PAD << " " << TotalLayerThickness << G4endl;
            G4ThreeVector trans_PAD = G4ThreeVector(Start_X, Start_Y, position_Z_PAD);
            assemblyPad1->MakeImprint(envelope, trans_PAD, 0, false, false); // assemblyPad->MakeImprint(envelope, trans_PAD, 0, false, true);
    ...
    // Change Pad layer IDs
    new_id = IDnumber_PAD_First1;
    std::vector<G4VPhysicalVolume *>::iterator iter1 = assemblyPad1->GetVolumesIterator();
    for (int i = 0; i < (int)assemblyPad1->TotalImprintedVolumes(); iter1++, i++)
    {
        G4VPhysicalVolume *ptr = *iter1;
        if (ptr)
        {
            // this was a very optimistic way of checking if it should be a scoring volume or not
            // the inactive part of the silicon is sensor is also of material "G4_Si"
            ...
            if (ptr->GetLogicalVolume()->GetMaterial()->GetName() == "G4_Si")
            {
                ...
                if (new_id % 2)
                {
                    ...
                    ptr->SetCopyNo(1 + new_id / 2);
                    //  ptr->SetCopyNo(new_id);
                    G4cout << ptr->GetCopyNo() << "   name : " << ptr->GetName() << "  " << (int)assemblyPad1->TotalImprintedVolumes() << G4endl;
                }
                else
                {
                    // if even, set zero
    ...
    for (int i = 0; i < (int)assemblyPixel->TotalImprintedVolumes(); iter++, i++)
    {
        ...
        if (ptr)
        {
            if (ptr->GetLogicalVolume()->GetMaterial()->GetName() == "G4_Si")
            {
                ptr->SetCopyNo(new_id);
                new_id++;
                G4cout << ptr->GetCopyNo() << "   name1 : " << ptr->GetName() << G4endl;
            }
            else
            {
                ptr->SetCopyNo(0);
...
G4AssemblyVolume *Geometry::ConstructPadLayer()
{
    ...
    logVol_PAD_Silicon_inactive->SetVisAttributes(G4Color::Red()); // Green
...
G4AssemblyVolume *Geometry::ConstructPadLayer1()
{
    G4AssemblyVolume *assemblyPad = new G4AssemblyVolume();
...
void Geometry::CreateMaterials(G4NistManager *manager)
{
    ...
    material_tungsten->AddElement(elW, 94 * perCent); // the percentage of material
